...
export default async function AdminAnalyticsPage() {
  ...
  let funnel: {
    ...
    checkouts: number;
    purchases: number;
  } = { productViews: 0, cartAdds: 0, checkouts: 0, purchases: 0 };
  let cohorts: { cohortMonth: string; newCustomers: number }[] = [];

  try {
    const caller = await api();
    [revenue, topProducts, funnel, cohorts] = await Promise.all([
      caller.admin.analyticsRevenue({ days: 30 }),
      caller.admin.analyticsTopProducts({ limit: 10 }),
      caller.admin.analyticsFunnel(),
      caller.admin.analyticsCohorts(),
    ]);
  } catch (err) {
        ...
        {revenue.length === 0 ? (
          ...
          >
            {revenue.map((r) => (
              <div
                key={r.date}
                title={`${formatDate(r.date)}: ${formatPrice(r.revenueCents)} (${String(r.orderCount)} orders)`}
                style={{
                  flex: 1,
                  height: `${String((r.revenueCents / maxRevenue) * 100)}%`,
                  background: 'var(--clay)',
                  minHeight: 2,
