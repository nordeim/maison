    ...
    return (
      <div>
        ...
        <p style={{ color: 'var(--muted)' }}>
          Unable to load dashboard data. Make sure the database is configured and seeded.
        </p>
      </div>
    );
  }
        ...
        {[
          {
            ...
            value: formatPrice(overview.todayRevenueCents),
          },
          { label: "Today's Orders", value: String(overview.todayOrderCount) },
          {
            label: 'Average Order Value',
            value: formatPrice(overview.aovCents),
          },
          {
            label: 'All-Time Revenue',
            value: formatPrice(overview.allTimeRevenueCents),
          },
        ].map((kpi) => (
          <div
            key={kpi.label}
                ...
                >
                  <div>
                    ...
                    <p style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>
                      {order.email} · {order.placedAt ? formatDate(order.placedAt) : '—'}
                    </p>
                  </div>
              ...
              {overview.lowStockVariants.map((variant) => (
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 11,
                      color: variant.stockQuantity === 0 ? 'var(--clay)' : 'var(--warning)',
                    }}
                  >
