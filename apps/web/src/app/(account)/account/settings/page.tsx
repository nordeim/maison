...
export default function AccountSettingsPage() {
  ...
  const handleProfileSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      void utils.account.getProfile.invalidate();
  ...
  const handleDeleteAccount = () => {
    ...
    setError(
      'Account deletion is not yet available. Please contact hello@maison-living.com to request deletion.',
    );
  };
      ...
      <section style={{ marginBottom: '3rem' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            {profileSaved && (
              <span style={{ fontSize: '0.875rem', color: 'var(--sage)' }}>✓ Saved</span>
            )}
      ...
      <section style={{ marginBottom: '3rem' }}>
        ...
        >
          <input
            ...
            onChange={(e) => void handleNewsletterToggle(e.target.checked)}
            style={{ width: 18, height: 18, accentColor: 'var(--clay)' }}
          />
          <span style={{ fontSize: '0.9375rem', color: 'var(--ink-2)' }}>
            Subscribe to <strong>Letters from Maison</strong> — one quiet email a month with new
            pieces, workshop notes, and early access to small-batch releases.
          </span>
        </label>
      </section>
