...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  if (query.length >= 2) {
    try {
      results = await api().products.search({ q: query, limit: 24 });
    } catch (err) {
  ...
  return (
    <main style={{ maxWidth: "var(--container)", margin: "0 auto", padding: "5rem var(--gutter)" }}>
      <div style={{ marginBottom: "3rem" }}>
        <span style={{ display: "inline-block", fontSize: 11, fontWeight: 500, letterSpacing: "0.22em", textTransform: "uppercase", color: "var(--clay)", marginBottom: "1rem" }}>Search</span>
        <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(2rem, 4.5vw, 3.4rem)", fontWeight: 500 }}>
          {query ? (
      ...
      {!query ? (
        <div style={{ textAlign: "center", padding: "4rem 0", color: "var(--muted)" }}>
          <p style={{ marginBottom: "1.5rem" }}>Use the search bar at the top of the page to find pieces by name, material, or description.</p>
          <Link href="/products" style={{ color: "var(--clay)", borderBottom: "1px solid var(--clay)" }}>
            Or browse all pieces →
          </Link>
        </div>
      ) : results.length === 0 ? (
        <div style={{ textAlign: "center", padding: "4rem 0", color: "var(--muted)" }}>
          <p style={{ fontFamily: "var(--font-serif)", fontSize: "1.5rem", color: "var(--ink)", marginBottom: "1rem" }}>
            No pieces match "{query}"
          </p>
          <p style={{ marginBottom: "1.5rem" }}>Try a different search term, or browse our full collection.</p>
        ...
        <div className="search-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1.75rem 1.25rem" }}>
          {results.map((p) => (
            <ProductCard
              key={p.id}
              product={{
