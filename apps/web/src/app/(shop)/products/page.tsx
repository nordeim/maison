  ...
  return (
    <main style={{ maxWidth: "var(--container)", margin: "0 auto", padding: "5rem var(--gutter)" }}>
      ...
      <div style={{ marginBottom: "3rem" }}>
        <span style={{ display: "inline-block", fontSize: 11, fontWeight: 500, letterSpacing: "0.22em", textTransform: "uppercase", color: "var(--clay)", marginBottom: "1rem" }}>Shop All</span>
        <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(2rem, 4.5vw, 3.4rem)", fontWeight: 500 }}>
          {activeCollection && activeCollection.slug ? activeCollection.name : "All Pieces"}
        </h1>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "3rem", flexWrap: "wrap", gap: "1rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          <label style={{ fontSize: 12, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.1em" }} htmlFor="sort">
            Sort
          </label>
      ...
      {products.length === 0 ? (
        <div style={{ textAlign: "center", padding: "4rem 0", color: "var(--muted)" }}>
          ...
          <Link href="/products" style={{ display: "inline-block", marginTop: "1.5rem", color: "var(--clay)", borderBottom: "1px solid var(--clay)", paddingBottom: "2px" }}>
            View all pieces →
          </Link>
        </div>
      ) : (
        <div className="plp-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1.75rem 1.25rem" }}>
          {products.map((p) => (
