...
export const metadata: Metadata = {
  title: "Collections",
  description: "Browse our curated collections — Lighting, Furniture, Textiles, Ceramics, and more.",
};

export default async function CollectionsPage() {
  let collections: Array<{ slug: string; name: string; description: string | null }> = [];
      ...
      {collections.length === 0 ? (
        ...
        >
          {collections.map((c) => (
            ...
            >
              <h3 style={{ fontFamily: "Georgia, serif", fontSize: "1.5rem", fontWeight: 500, marginBottom: "0.5rem" }}>
                {c.name}
              </h3>
              {c.description && (
                <p style={{ fontSize: "0.875rem", color: "#4a433b", lineHeight: 1.65 }}>
                  {c.description}
                </p>
              )}
              <p style={{ marginTop: "1rem", fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", color: "#a86b4a" }}>
                View pieces →
              </p>
            </a>
          ))}
