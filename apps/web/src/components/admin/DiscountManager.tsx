...
export function DiscountManager({ initialDiscounts }: { initialDiscounts: Discount[] }) {
  ...
  const handleCreate = async (e: React.SubmitEvent) => {
    ...
    try {
      ...
      void utils.admin.discountsList.invalidate();
      setForm({
        code: '',
        type: 'percentage',
        value: '',
    ...
    <div>
      ...
      >
        <p style={{ fontSize: '0.875rem', color: 'var(--muted)' }}>
          {initialDiscounts.length} discount codes
        </p>
        <button
          onClick={() => {
        ...
        >
          ...
          >
            ...
            <div>
              ...
              >
                Value *{' '}
                {form.type === 'percentage' ? '(%)' : form.type === 'fixed' ? '($)' : '(n/a)'}
              </label>
              <input
                type="number"
                value={form.value}
                ...
                placeholder={form.type === 'percentage' ? '10' : form.type === 'fixed' ? '25' : '—'}
                style={{
                  width: '100%',
                  padding: '0.5rem 0.75rem',
                  border: '1px solid var(--line)',
            ...
            <tbody>
              {initialDiscounts.map((d) => (
                <tr key={d.id} style={{ borderBottom: '1px solid var(--line-soft)' }}>
                  ...
                  <td style={{ padding: '0.75rem 1rem', color: 'var(--ink-2)' }}>
                    {d.type.replace('_', ' ')}
                  </td>
                  ...
                  <td style={{ padding: '0.75rem 1rem', color: 'var(--muted)' }}>
                    {d.minOrderCents > 0 ? formatPrice(d.minOrderCents) : '—'}
                  </td>
                  <td style={{ padding: '0.75rem 1rem', color: 'var(--muted)' }}>
                    {d.usesCount}
                    {d.maxUses ? ` / ${String(d.maxUses)}` : ''}
                  </td>
                  <td style={{ padding: '0.75rem 1rem' }}>
                    <span
                      style={{
                        ...
                        textTransform: 'uppercase',
                        letterSpacing: '0.1em',
                        color: d.isActive ? 'var(--sage)' : 'var(--muted)',
                      }}
                    >
