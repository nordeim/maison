        ...
        <div className="jour-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "2rem" }}>
          {ARTICLES.map((a) => (
            <a key={a.slug} href={`/journal/${a.slug}`} style={{ display: "block", transition: "transform 0.45s var(--ease-maison)" }}>
              <div style={{ aspectRatio: "4 / 3", overflow: "hidden", marginBottom: "1.5rem", background: "var(--bg-2)", position: "relative" }}>
                <Image src={a.image} alt="" fill sizes="(max-width: 768px) 100vw, 33vw" loading="lazy" style={{ objectFit: "cover", transition: "transform 1s var(--ease-maison)" }} />
              </div>
