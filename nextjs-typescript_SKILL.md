...
description: Monorepo web projects using pnpm, Turborepo, TypeScript, Next.js, React, ESLint, Prettier, Drizzle, Postgres, and third-party SDKs. Prevent repeated mistakes and provide a reusable troubleshooting methodology.
version: 1.0
---
...
### Mistake 2: Assuming `.prettierrignore` is loaded when `--ignore-path` is passed
...
then Prettier may not automatically load `.prettierrignore`.
...
> Explicit ignore-path flags can override default ignore discovery.
...
5. Use TypeScript parser diagnostics if available.
6. Apply minimal syntax fix.
...
6. Run migrate in isolation.
7. Verify database state before and after.
8. Fix journal or remove bad generated migration.
9. Remove generate from setup if unsafe.
...
| DEP-5 | jest-dom deprecation | Caret admitted deprecated version | Exact pin | Pin sensitive deps deliberately |
