...
- **No PII in logs (Skill 2 §13.10, locked in v1.2.6 V9-1).** Never `console.log` user-supplied PII (name, email, message body). Use PII-redacted messages like `'[contact] Submission received (PII redacted)'`. The same Skill 2 §13.10 rule bans logging Stripe webhook payloads — log only event IDs + types, never request bodies. The `contact.submit` and `newsletter.subscribe` routers are the canonical examples (the v9 fix replaced their PII-leaking `console.log` calls).
