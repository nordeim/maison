...
1. **Hero** — full-viewport (`100svh`) background image with slow Ken Burns zoom, gradient overlay, kicker "Curated for Considered Living", H1 "Objects of _Quiet Beauty_", supporting paragraph, primary CTA "Shop Now" → `/products`, animated "Scroll" indicator.
...
- Current live behavior: **not a real payment checkout** — a banner reads _"Online checkout coming soon. Please submit your order request below and we'll contact you to complete your purchase."_ Form collects Contact Info (first/last name, email, phone), Shipping Address (street, city, postal code, country), Order Notes, and on submit shows a success toast, clears cart, redirects home — **no actual order is persisted, no payment is taken, no email is sent** (mocked with a 1.5s delay).
...
- Hero: kicker "Our Story", H1 "Curating Beauty _for Living_", subtext, full-bleed image.
