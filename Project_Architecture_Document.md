...
- **V15-1 — Fixed production build failure from V14's `useSearchParams()`
  ...
  `IntersectionObserver` setup is the only side-effect). Architecturally this
  is a static-prerendering / Suspense-boundary composition fix at the layout
  level — it does NOT touch the rendering-strategy contract (the `(shop)`
  layout was already a Server Component, `ScrollRevealTrigger` was already a
  `'use client'` Client Component, and the `(shop)` route group was already
  split between statically-prerendered public pages and the client-side
  `CartProvider` / `CartDrawer` — the only change is that the trigger is now
  suspended so `useSearchParams()`'s dynamic-rendering bailout does not
  propagate to the statically-prerendered pages that transitively render
  it), the Layer-1/2/3 security model, the Client/Server Component boundary,
  the RSC/streaming posture, the `proxy.ts` Layer-1 invariant, or any
  procedure tier. No changes to `useScrollReveal.ts`, the
  `ScrollRevealTrigger` Client Component itself, the `.reveal` / `.visible`
  utilities, the design tokens, the section structure, the copy, or any
  keyframe. The V11-1 + V12-1 + V14-1 wiring inside the hook is unchanged.
